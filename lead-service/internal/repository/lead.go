...
import (
	...
	"github.com/fiveret/crm-golang/internal/lead"
	"github.com/fiveret/crm-golang/internal/models"
	"github.com/gofiber/fiber"
)

func GetLead(db *db.DBConnection) fiber.Handler {
...
func GetLeads(db *db.DBConnection) fiber.Handler {
	return func(c *fiber.Ctx) {
		leads := db.FindLeads()
...
func DeleteLead(db *db.DBConnection) fiber.Handler {
	return func(c *fiber.Ctx) {
		id := c.Params("id")
...
func NewLead(db *db.DBConnection) fiber.Handler {
	return func(c *fiber.Ctx) {
...
func UpdateLead(db *db.DBConnection) fiber.Handler {
	return func(c *fiber.Ctx) {
		id := c.Params("id")
		if id == "" {
